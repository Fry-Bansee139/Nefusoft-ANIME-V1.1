...
const Home = () => {
  ...
  const handleShare = async (platform) => {
    ...
    if (platform === 'api') {
      if (navigator.share) {
        try { await navigator.share({ title: 'Alfeent', text: text, url }); } catch (e) {}
      }
      return;
    }
        ...
        <div className="relative bg-[#16161a] p-6 md:p-8 rounded-xl border border-white/5 overflow-hidden shadow-xl">
           ...
           <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5">
             <div>
               ...
               <p className="text-white/60 text-[10px] md:text-xs font-medium">Ajak teman-temanmu marathon anime favorit bareng di Alfeent.</p>
             </div>
